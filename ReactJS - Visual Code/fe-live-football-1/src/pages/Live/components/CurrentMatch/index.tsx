...
const CurrentMatch = ({ match }: { match: Match }) => {
  ...
  return (
    <div className="w-full relative bg-[url('/bg-match-card.png')] bg-cover bg-center bg-no-repeat rounded-[12px] border-2 border-white overflow-hidden">
      ...
      <div className="w-full py-[calc(32px+8px)] md:py-[calc(32px+16px)] px-[8px] grid grid-cols-[1fr_auto_1fr]">
        ...
        <div className="flex items-center h-full justify-center">
          {match.isPlaying ? (
            ...
            <div className="flex flex-col items-center justify-center gap-2 text-[16px] md:text-[20px] font-semibold whitespace-nowrap">
              <p>{onlyTime}</p>
              <p>{onlyDate}</p>
            </div>
          )}
      ...
      <div className="absolute bottom-0 w-full h-[32px]">
        <div
          className={clsx(
            "absolute bottom-0 left-1/2 -translate-x-1/2 h-full flex items-center px-4 text-[14px] rounded-tr-[8px] rounded-tl-[8px] border-l-2 border-r-2 border-t-2 border-white",
            "justify-center min-w-[96px] bg-gradient-to-r from-[#080247] to-[#2A4AE1]"
          )}
        >
          <span>{match.commentator.nickname}</span>
        </div>
      </div>
    </div>
  );
