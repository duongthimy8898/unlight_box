  ...
  return (
    <section className="mt-2">
      <ul className="flex flex-row gap-1.5 sport-list">
        <li className={clsx("shrink-0 border-b border-b-transparent rounded overflow-hidden", "bg-linear-to-r from-[#0084ff]/0 via-[#0084ff] to-[#0084ff]/0")}>
          <Link
            to="/"
            activeOptions={{
              exact: true,
          ...
          >
            <img src={"/favicon.svg"} alt="Tất cả" className="size-5 object-contain rounded-full" />
        ...
        {sports.sort((a, b) => Number(b.hasLive) - Number(a.hasLive)).map((sport, idx) => (
          <li
            key={idx}
            className={clsx("relative shrink-0 border-b border-b-transparent rounded overflow-hidden", "bg-linear-to-r from-[#0084ff]/0 via-[#0084ff] to-[#0084ff]/0")}
          >
            {sport.hasLive && (
