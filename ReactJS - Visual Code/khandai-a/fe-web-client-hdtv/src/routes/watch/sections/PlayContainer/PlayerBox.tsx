        ...
        layers: [
          {
            ...
            html: `
                      <div id="video-overlay-bottom" style="width:100%;overflow:hidden;position:relative;">
                        <div class="w-full flex justify-end items-center px-1 mb-1 lt:px-2 lt:mb-2">
                          <div class="w-fit flex gap-1">
                             <a
                                  href="https://www.olabet.today/register?affiliateCode=hoadaotv"
                                  target="_blank"
                                  style="color: oklch(90.5% 0.182 98.111)"
                                  class="bg-green-500 text-yellow-300 font-['system-ui'] uppercase font-bold py-0.5 px-1.5 md:py-1 md:px-2 rounded transition-colors text-[10px] md:text-[12px]"
                                >Cược OLA BET</a>
                          </div>
                        </div>
                      </div>
    ...
    <div className="bg-[#0F0F0F] flex flex-col">
      ...
      >
        <div className="flex-1 min-w-0 flex flex-row items-center gap-1">
          ...
          <div className="w-full flex flex-col overflow-hidden">
            ...
            <p className="text-xs text-pink-500">{props.broadcasts?.at(0)?.commentator.nickname}</p>
          </div>
        </div>
        <div className="w-fit shrink-0 flex flex-row gap-2 justify-end">
          <div className="flex flex-row items-center gap-1">
            ...
            <a
              href="https://t.me/+0faUSH5QN_hjYjQ0"
              target="_blank"
              className="flex flex-row items-center justify-center gap-1 pr-3 rounded-full font-semibold bg-pink-400"
            >
      ...
      {props.status === "READY" ? (
        ...
        <div key="poster" className={clsx("bg-black flex-1 aspect-video relative", state.cleanMode && "aspect-video lg:aspect-auto! object-contain!")}>
          ...
          >
            <div className="p-4 lt:p-16 absolute top-6/10 left-1/2 -translate-y-1/2 -translate-x-1/2 w-full">
              {/* <Countdown targetDate={props.startTime ?? new Date()} /> */}
              <div className="w-full text-center">
                <div className="flex items-center justify-center">
                  <Countdown targetDate={props.startTime} />
                </div>
              </div>
            </div>
          </div>
      ...
      <a href="https://www.olabet.today/register?affiliateCode=hoadaotv" target="_blank" className="block shrink-0">
        <img className="block aspect-1920/60 w-full h-auto bg-slate-300" src="/banners/ola_1920x60.gif" alt="" />
      </a>
    </div>
  );
};
