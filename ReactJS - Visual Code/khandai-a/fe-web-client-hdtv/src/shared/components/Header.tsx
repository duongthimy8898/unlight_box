  ...
  return (
    <header className="fixed z-50 top-0 left-0 right-0 h-16 bg-surface border-b bg-[#191919] border-[#2a2a2e] px-2 md:px-5 lg:px-34 py-2 flex items-center justify-between shrink-0">
      <a href="/" className="h-full w-auto">
        <img src="/logo-x.png" alt="Hoa Đào TV" className="w-auto h-full" />
      </a>
      ...
      <nav className="hidden sm:flex gap-0.5 items-center">
        {NAV_ITEMS.map((item) => (
          <Link
            ...
            activeProps={{
              className: "text-white! bg-[#ff5c8d]!",
            }}
          >
            {item.icon} {item.text}
          </Link>
      ...
      <div className="flex items-center gap-2">
        <a
          ...
          target="_blank"
          className="flex space-x-1 px-3.5 py-1.5 rounded-md bg-linear-to-r from-[#973201] to-[#ff5c8d] hover:from-pink-400 hover:to-pink-700 transition-colors"
        >
