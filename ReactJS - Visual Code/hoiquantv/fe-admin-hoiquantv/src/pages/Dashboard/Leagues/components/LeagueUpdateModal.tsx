...
const InternalUserUpdateModal = ({ isOpening, targetId, onClose }: Props) => {
    ...
    const fetchData = async () => {
      ...
      try {
        const res: AxiosResponse<{ code: number; message: string; data: League }> = await axios.get(
          `${env.VITE_SERVER_API_BASE_URL}/leagues/get?by=id&value=${targetId}`
        );
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.put(`${env.VITE_SERVER_API_BASE_URL}/leagues/update/${targetId}`, submitData);
      await sleep(500);
        ...
        >
          ...
          >
            ...
            <div className="w-full py-5 px-4 bg-yellow-500/10 font-semibold flex-shrink-0 uppercase text-yellow-500">
              <span>Chỉnh sửa thông tin giải đấu {leagueData?.sport.name}</span>
              <span> - </span>
              <span>Mã giải đấu #{leagueData?.id}</span>
            </div>
