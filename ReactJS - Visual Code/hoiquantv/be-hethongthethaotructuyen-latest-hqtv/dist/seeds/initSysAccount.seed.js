...
async function initSysAccount() {
    ...
    const hashedPassword = await bcryptjs_1.default.hash(process.env.ADMIN_PASSWORD || "sys_ad@hoiquan.tv!5602", 10);
    const admin = repositories_1.internalUserRepo.create({
        username: process.env.ADMIN_USERNAME || "sys_ad",
        email: process.env.ADMIN_EMAIL || "sys_ad@hoiquan.tv",
        name: "Sys Admin",
