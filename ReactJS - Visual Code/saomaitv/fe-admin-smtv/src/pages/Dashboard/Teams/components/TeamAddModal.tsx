...
const TeamAddModal = ({ isOpening, sportId, onClose }: Props) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post(`${env.VITE_SERVER_API_BASE_URL}/teams/create`, submitData);
      await sleep(500);
        ...
        >
          ...
          >
            ...
            <div className="w-full py-5 px-4 bg-yellow-500/10 font-semibold flex-shrink-0 uppercase text-yellow-500">
              <span>Thêm đội tuyển {sportData?.name}</span>
            </div>
