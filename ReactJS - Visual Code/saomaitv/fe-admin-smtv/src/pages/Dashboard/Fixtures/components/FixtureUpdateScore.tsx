...
import type { Fixture } from "../../../../types/data.type";
...
const FixtureUpdateScoreModal = ({ isOpening, fixtureId, onClose }: Props) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.put(`${env.VITE_SERVER_API_BASE_URL}/fixture-scores/update/${fixtureData?.score?.id}`, submitData);
      await sleep(500);
        ...
        >
          ...
          >
            ...
            <div className="w-full py-5 px-4 bg-yellow-500/10 font-semibold flex-shrink-0 uppercase text-yellow-500">
              <span>CẬP NHẬT TỈ SỐ {fixtureData?.sport?.name}</span>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="w-full flex flex-col gap-2">
                    <label htmlFor="home" className="text-white text-xs">
                      ĐỘI CHỦ NHÀ ({fixtureData?.homeTeam.name})
                    </label>
                    <input
                      className="inline-block p-2 bg-[#1c2433] border border-white/10 outline-2 -outline-offset-1 outline-transparent focus:outline-blue-500 text-white font-semilight text-sm"
                      type="number"
                      id="home"
                      name="home"
                      defaultValue={fixtureData?.score?.home}
                    />
                  </div>
                  <div className="w-full flex flex-col gap-2">
                    <label htmlFor="away" className="text-white text-xs">
                      ĐỘI KHÁCH ({fixtureData?.awayTeam.name})
                    </label>
                    <input
                      className="inline-block p-2 bg-[#1c2433] border border-white/10 outline-2 -outline-offset-1 outline-transparent focus:outline-blue-500 text-white font-semilight text-sm"
                      type="number"
                      id="away"
                      name="away"
                      defaultValue={fixtureData?.score?.away}
                    />
                  </div>
                </div>
...
export default FixtureUpdateScoreModal;
