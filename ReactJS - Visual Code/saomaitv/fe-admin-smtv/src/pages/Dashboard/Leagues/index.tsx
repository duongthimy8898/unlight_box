...
const LeaguesPage = () => {
  ...
  const fetchLeagues = useCallback(async () => {
    try {
      const res = await axios.get<{ data: League[] }>(`${env.VITE_SERVER_API_BASE_URL}/leagues`);
  ...
  useEffect(() => {
    setFilteredLeagues(currentSport?.id === undefined ? leagues : leagues.filter((l) => l.sport.id === currentSport?.id));
  }, [currentSport, leagues]);

  const [processingId, setProcessingId] = useState<number | null>(null);
  const [isLeagueAddModalOpen, setLeagueAddModal] = useState(false);
  const [isLeagueUpdateModalOpen, setLeagueUpdateModal] = useState(false);

  const handleAdd = () => {
    if (currentSport?.id) {
      setProcessingId(currentSport.id ?? null);
      setLeagueAddModal(true);
    } else toast.error("Phải chọn 1 bộ môn nào đó đã :))", { closeButton: false });
  };

  const handleEdit = useCallback((id: number) => {
    setProcessingId(id);
    setLeagueUpdateModal(true);
  }, []);
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      if (confirm("Bạn có chắc muốn xóa giải này?")) {
        ...
        try {
          const response = await axios.delete(`${env.VITE_SERVER_API_BASE_URL}/leagues/delete/${id}`);
          // await sleep(500);
  ...
  const columns: ConfigColumns[] = [
    {
      ...
      render: (row: League) =>
        ` <div class="flex flex-col-reverse gap-1 items-center text-xs">
            <img src="${row.logoUrl}" alt="" class="border w-8 h-8 md:w-12 md:h-12 inline-block" />
          </div>`,
    },
    { title: "#", data: "id" },
    { title: "Tên giải đấu", data: "name" },
  ...
  return (
    <>
      <div className="w-full shadow p-4 bg-gray-100">
          ...
          slots={{
            actions: (row: League) => (
              <div className="flex gap-2 whitespace-nowrap">
                ...
                >
                  <LuTrash />
                  <span className="text-xs font-semibold">Xóa giải</span>
                </button>
              </div>
            ),
          }}
        ...
        />
        <LeagueAddModal
          isOpening={isLeagueAddModalOpen}
          sportId={processingId}
          onClose={() => {
            setLeagueAddModal(false);
            fetchLeagues();
          }}
        />
        <LeagueUpdateModal
          targetId={processingId}
          isOpening={isLeagueUpdateModalOpen}
          onClose={() => {
            setProcessingId(null);
            setLeagueUpdateModal(false);
            fetchLeagues();
          }}
        />
      </div>
