...
const InteractBox = ({
  ...
  const fixtureStats = useFixtures.listStats().data?.find((fs: FixtureStats) => fs.fixture.id.toString() === referenceId);
  return (
    <div className={clsx("bg-transparent w-full flex flex-col overflow-hidden", "h-120 lg:h-full shrink-0 lg:flex-1", isClientViewMode && "flex-1!")}>
        ...
        {[
          ...
          <button
            ...
            className={`py-1.5 rounded text-xs font-medium flex items-center justify-center transition-all duration-200 hover:opacity-100 overflow-hidden
              ${
                activeTab === tab.id
                  ? "bg-orange-500 text-white border border-orange-500 px-4 gap-1 opacity-100 w-fit max-w-96 h-7"
                  : "bg-white/20 text-white border border-gray-300 px-0 w-10 max-w-10 h-7 opacity-80"
              }
            `}
          >
      ...
      {/* Content */}
      <div className={clsx("h-full flex-1 bg-[linear-gradient(180deg,#282a2c_0.08%,#282a2c_10.42%,#1e2021_40.82%)]")}>
        {activeTab === "tab1" && (
          <div className={clsx("w-full h-full flex flex-col")}>
            <div className="flex-1 flex flex-col relative">
              <div className="flex">
                <button
                  ...
                  className={clsx(
                    ...
                    chatMode === "global" ? "border-orange-500 text-orange-500" : "border-white opacity-80",
                  )}
                >
                  Chung
                </button>
                <button
                  ...
                  className={clsx(
                    "flex-1 py-1.5 px-3 text-xs font-semibold border-b-2",
                    chatMode === "local" ? "border-orange-500 text-orange-500" : "border-white opacity-80",
                  )}
                >
                  Trận
                </button>
              ...
              <div className="h-full" style={{ display: chatMode === "global" ? "block" : "none" }}>
                <iframe width="100%" height="100%" src="https://www5.cbox.ws/box/?boxid=962707&boxtag=YqPAde"></iframe>
              </div>
              <div className="h-full" style={{ display: chatMode === "local" && chatChannelId && chatChannelKey ? "block" : "none" }}>
                <iframe
                  width="100%"
                  height="100%"
                  src={`https://www5.cbox.ws/box/?boxid=962707&boxtag=YqPAde&tid=${chatChannelId}&tkey=${chatChannelKey}`}
                ></iframe>
              </div>
        ...
        {activeTab === "tab2" && (
          <div className="w-full h-full">
            <iframe
              width="100%"
              height="100%"
              src={`https://widget.livestats.online/?fixtureId=${referenceId}&primaryColor=%23ff6900&secondaryColor=%23FF9900`}
            ></iframe>
          </div>
        )}
        {activeTab === "tab3" && (
          <div className="w-full h-full">
            <iframe
              width="100%"
              height="100%"
              src={`https://widget.livestats.online/standings.html?leagueId=${fixtureStats?.league.id}&season=${fixtureStats?.league.season}&primaryColor=%23ff6900&secondaryColor=%23FF9900`}
            ></iframe>
          </div>
        )}
        {activeTab === "tab4" && (
          <div className="w-full h-full">
            <iframe
              width="100%"
              height="100%"
              src={`https://widget.livestats.online/h2h.html?h2h=${fixtureStats?.teams.home.id}-${fixtureStats?.teams.away.id}&primaryColor=%23ff6900&secondaryColor=%23FF9900`}
            ></iframe>
          </div>
        )}
      </div>
