...
async function initChat() {
  const fs = (await fixtureService.getAllFixtures()).filter(
    (f) => f.chatChannelKeyId === null
  );
  for (const fixture of fs) {
    try {
      ...
      console.log(
        "Chat initialized for fixture ID:",
        fixture.id,
        "with chatChannelKeyId:",
        chatChannelKeyId
      );
    } catch (error) {
      console.error(
        "Error initializing chat for fixture ID:",
        fixture.id,
        error
      );
    }
