...
async function mapToMatch(raw: any): Promise<Match> {
  ...
  // xIsPlaying = raw.streams.length > 0;
  // console.log("map", raw);

  // if (raw.referenceId) {
  //   try {
  //     const res = await fetch(`https://api.livestats.online/api/v1/fixtures/${raw.slug}-I${raw.referenceId}`, {
  ...
  return {
    ...
    commentator: {
      ...
      avatar: raw.commentator?.avatarUrl || "/favicon.png",
...
function mapApiToMatch(raw: any): Match {
  return {
    ...
    commentator: {
      nickname: raw.commentator?.nickname || "Nhà đài",
      avatar: raw.commentator?.avatarUrln || "/favicon.png",
      streamSourceSd: raw.commentator?.streamSourceSd,
      streamSourceHd: raw.commentator?.streamSourceHd,
      streamSourceFhd: raw.commentator?.streamSourceFhd,
    },
