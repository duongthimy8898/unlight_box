"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.sportService = void 0;
const repositories_1 = require("../../db/repositories");
exports.sportService = {
    async getAllSports() {
        return await repositories_1.sportRepo
            .createQueryBuilder("sport")
            // Đếm số fixture chưa FT
            .loadRelationCountAndMap("sport.fixtureCount", "sport.fixtures", "fixture", (qb) => qb
            .leftJoin("fixture.status", "status")
            .where("status.code IS NULL OR status.code != :code", {
            code: "FT",
        }))
            // Đếm số fixture đang LIVE
            .loadRelationCountAndMap("sport.liveFixtureCount", "sport.fixtures", "liveFixture", (qb) => qb.where("liveFixture.isLive = :live", { live: true }))
            .loadRelationCountAndMap("sport.liveFixtureCount", "sport.fixtures", "liveFixture", (qb) => qb.where("liveFixture.isLive = :live", { live: true }))
            .getMany()
            .then((sports) => sports.map((s) => ({
            ...s,
            hasLive: s.liveFixtureCount > 0, // nếu có trận đang live
        })));
    },
    async getSport(by, value) {
        const where = by === "id" ? { id: Number(value) } : { slug: String(value) };
        const sport = await repositories_1.sportRepo
            .createQueryBuilder("sport")
            .where(where)
            // đếm số fixture chưa FT
            .loadRelationCountAndMap("sport.fixtureCount", "sport.fixtures", "fixture", (qb) => qb
            .leftJoin("fixture.status", "status")
            .where("status.code IS NULL OR status.code != :code", {
            code: "FT",
        }))
            // đếm số fixture đang LIVE
            .loadRelationCountAndMap("sport.liveFixtureCount", "sport.fixtures", "liveFixture", (qb) => qb.where("liveFixture.isLive = :live", { live: true }))
            .getOne();
        if (!sport)
            return null;
        // thêm field isLive dựa trên liveFixtureCount
        return {
            ...sport,
            hasLive: sport.liveFixtureCount > 0,
        };
    },
};
