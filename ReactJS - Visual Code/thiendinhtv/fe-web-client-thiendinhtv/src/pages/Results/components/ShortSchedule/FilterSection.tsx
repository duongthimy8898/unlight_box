...
const FilterSection = ({ selectedFilter }: { selectedFilter: { by: string | null; value: string | null } }) => {
  const filters = [
    ...
    { label: "H.Qua", value: dayjs().add(-1, "day") },
    ...
    <div className={clsx("sticky top-0 bg-[#141414] py-4 flex flex-row gap-2 overflow-auto", "lt:flex-col")}>
      ...
      {filters.map((filter, idx) => (
        <Link
          ...
          className={clsx(
            "py-2 px-6 rounded-[12px] flex flex-col gap-1 items-center hover:bg-[#e2fe35] hover:text-black transition-all",
            selectedFilter.by === "date" && selectedFilter.value === filter.value.format("DD/MM/YYYY") ? "bg-[#e2fe35] text-black" : "bg-gray-500/25 text-white"
          )}
        >
