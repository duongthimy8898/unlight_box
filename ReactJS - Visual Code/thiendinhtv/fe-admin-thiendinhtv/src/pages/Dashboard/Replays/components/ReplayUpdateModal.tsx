...
import type { Replay } from "../../../../types/data.type";
...
const InternalUserUpdateModal = ({ isOpening, targetId, onClose }: Props) => {
  const [replayData, setReplayData] = useState<Replay | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setReplayData(null);
      try {
        const res: AxiosResponse<{ code: number; message: string; data: Replay }> = await axios.get(
          `${env.VITE_SERVER_API_BASE_URL}/replays/get?by=id&value=${targetId}`
        );
        if (res.data.code !== 200) throw new Error(`Error code: ${res.data.code} - Message: ${res.data.message}`);
        setReplayData(res.data.data);
        if (res.data.data.thumbnailUrl != null) setLogoUrl(res.data.data.thumbnailUrl);
        else setLogoUrl(undefined);
      } catch (error) {
        console.error("Failed to fetch replay dataa:", error);
        setReplayData(null);
      }
    };
    fetchData();
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.put(`${env.VITE_SERVER_API_BASE_URL}/replays/update/${targetId}`, submitData);
      await sleep(500);
        ...
        >
          ...
          >
            ...
            <div className="w-full py-5 px-4 bg-yellow-500/10 font-semibold flex-shrink-0 uppercase text-yellow-500">
              <span>Chỉnh sửa thông tin giải đấu {replayData?.sport.name}</span>
              <span> - </span>
              <span>Mã giải đấu #{replayData?.id}</span>
            </div>
                  ...
                  <div className="w-full flex gap-4">
                    ...
                    <div className="flex-1 flex flex-col justify-start gap-2">
                      ...
                      <input
                        ...
                        id="logoUrl"
                        name="logoUrl"
                        defaultValue={replayData.thumbnailUrl ?? ""}
