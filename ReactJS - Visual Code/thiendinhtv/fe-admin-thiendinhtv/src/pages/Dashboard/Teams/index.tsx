import DataTable from "datatables.net-react";
import DT, { type ConfigColumns } from "datatables.net-dt";
import { useCallback, useEffect, useState } from "react";
import axios from "axios";
import "./index.css";
import TeamAddModal from "./components/TeamAddModal";
import TeamUpdateModal from "./components/TeamUpdateModal";
import { LuPencilLine, LuPlus, LuTrash } from "react-icons/lu";

import { toast } from "react-toastify";
import type { League, Sport, Team } from "../../../types/data.type";
import { Link, useSearchParams } from "react-router-dom";
import clsx from "clsx";
const env = import.meta.env;
const TeamsPage = () => {
  DataTable.use(DT);
  const [searchParams] = useSearchParams();
  const [sports, setSports] = useState<Sport[]>([]);
  const sportId = searchParams.get("sportId") || sports[0]?.slug || undefined;
  const [currentSport, setCurrentSport] = useState<Sport>();
  const [teams, setTeams] = useState<Team[]>([]);
  const [filteredTeams, setFilteredTeams] = useState<Team[]>([]);

  const fetchTeams = useCallback(async () => {
    try {
      const res = await axios.get<{ data: Team[] }>(`${env.VITE_SERVER_API_BASE_URL}/teams`);
      setTeams(res.data.data);
    } catch (error) {
      console.error("Lỗi khi fetch sport:", error);
    }
  }, []);

  useEffect(() => {
    fetchTeams();
  }, [fetchTeams]);

  useEffect(() => {
    const fetchSports = async () => {
      // console.log("reload");
      try {
        const res = await axios.get<{ data: Sport[] }>(`${env.VITE_SERVER_API_BASE_URL}/sports`);
        const rawFixtures = res.data.data;
        setSports(rawFixtures);
      } catch (error) {
        console.error("Lỗi khi fetch sports:", error);
      }
    };
    fetchSports();
  }, []);

  useEffect(() => {
    setCurrentSport(sports.find((s) => s.id.toString() === sportId));
  }, [sportId, sports]);

  useEffect(() => {
    setFilteredTeams(currentSport?.id === undefined ? teams : teams.filter((t) => t.sport.id === currentSport?.id));
  }, [currentSport, teams]);

  const [processingId, setProcessingId] = useState<number | null>(null);
  const [isTeamAddModalOpen, setTeamAddModal] = useState(false);
  const [isTeamUpdateModalOpen, setTeamUpdateModal] = useState(false);

  const handleAdd = () => {
    if (currentSport?.id) {
      setProcessingId(currentSport.id ?? null);
      setTeamAddModal(true);
    } else toast.error("Phải chọn 1 bộ môn nào đó đã :))", { closeButton: false });
  };

  const handleEdit = useCallback((id: number) => {
    setProcessingId(id);
    setTeamUpdateModal(true);
  }, []);

  const handleDelete = useCallback(
    async (id: number) => {
      if (confirm("Bạn có chắc muốn xóa môn này?")) {
        const toastId = toast.loading("Đang xóa...");
        try {
          const response = await axios.delete(`${env.VITE_SERVER_API_BASE_URL}/teams/delete/${id}`);
          // await sleep(500);
          const { code, message } = response.data;
          if (code !== 200) throw new Error(message || "Có lỗi xảy ra");
          toast.update(toastId, {
            render: message,
            type: "success",
            isLoading: false,
            autoClose: 250,
            onClose: () => fetchTeams(),
          });
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
        } catch (err: any) {
          toast.update(toastId, {
            render: err.message || "Lỗi không xác định",
            type: "error",
            isLoading: false,
            autoClose: 1500,
          });
        }
      }
    },
    [fetchTeams]
  );
  const columns: ConfigColumns[] = [
    {
      title: "",
      data: null,
      render: (row: Team) =>
        ` <div class="flex flex-col-reverse gap-1 items-center text-xs">
            <img src="${row.logoUrl}" alt="" class="border w-8 h-8 md:w-12 md:h-12 inline-block" />
          </div>`,
    },
    { title: "#", data: "id" },
    { title: "Tên đội", data: "name" },
    { title: "Actions", name: "actions", data: null },
  ];

  return (
    <>
      <div className="w-full shadow p-4 bg-gray-100">
        <div className="w-full flex gap-2 overflow-auto ">
          <Link
            to={`/dashboard/teams`}
            className={clsx(
              "flex gap-2 py-2 px-3 text-sm rounded font-semibold",
              currentSport?.id === undefined ? "bg-blue-500 text-white" : "bg-gray-300 text-black"
            )}
          >
            <span>Tất cả</span>
          </Link>
          {sports.map((s, idx) => (
            <Link
              key={idx}
              to={`?sportId=${s.id}`}
              className={clsx(
                "flex gap-2 py-2 px-3 text-sm rounded font-semibold",
                s.id === currentSport?.id ? "bg-blue-500 text-white" : "bg-gray-300 text-black"
              )}
            >
              {s.iconUrl && <img className="w-5 h-5" src={s.iconUrl} alt="" />}
              <span>{s.name}</span>
            </Link>
          ))}
        </div>
        <div className="w-full flex justify-between items-center py-2">
          <h1 className="text-xl">Đội tuyển {currentSport?.name}</h1>
          <button className="py-2 px-2 bg-green-700 flex items-center gap-1 whitespace-nowrap cursor-pointer text-white" onClick={handleAdd}>
            <LuPlus />
            <span className="text-xs font-semibold">Thêm mới</span>
          </button>
        </div>
        <DataTable
          options={{
            scrollX: true,
          }}
          slots={{
            actions: (row: League) => (
              <div className="flex gap-2 whitespace-nowrap">
                <button
                  className="py-2 px-2 bg-gray-200 flex items-center gap-1 whitespace-nowrap cursor-pointer text-gray-700"
                  onClick={() => handleEdit(row.id)}
                >
                  <LuPencilLine />
                  <span className="text-xs font-semibold">Chỉnh sửa</span>
                </button>

                <button
                  className="py-2 px-2 bg-red-500 flex items-center gap-1 whitespace-nowrap cursor-pointer text-white"
                  onClick={() => handleDelete(row.id)}
                >
                  <LuTrash />
                  <span className="text-xs font-semibold">Xóa giải</span>
                </button>
              </div>
            ),
          }}
          data={filteredTeams}
          columns={columns}
          className="table table-striped table-hover w-full text-sm text-left row-border text-gray-700"
        />
        <TeamAddModal
          isOpening={isTeamAddModalOpen}
          sportId={processingId}
          onClose={() => {
            setTeamAddModal(false);
            fetchTeams();
          }}
        />
        <TeamUpdateModal
          targetId={processingId}
          isOpening={isTeamUpdateModalOpen}
          onClose={() => {
            setProcessingId(null);
            setTeamUpdateModal(false);
            fetchTeams();
          }}
        />
      </div>
    </>
  );
};

export default TeamsPage;
